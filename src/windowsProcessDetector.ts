...
export class WindowsProcessDetector implements IPlatformStrategy {
    ...
    /**
     * Escape single quotes in PowerShell/WMIC strings to prevent injection
     */
    private escapeProcessName(name: string): string {
        // Only allow alphanumeric, underscore, hyphen, and dot
        // Also escape single quotes by doubling them for PowerShell/WMIC
    ...
    getProcessListCommand(processName: string): string {
        ...
        const safeName = this.escapeProcessName(processName);
        
        if (this.usePowerShell) {
            // PowerShell 命令:使用 Get-CimInstance 获取进程信息并输出 JSON
            return `powershell -NoProfile -Command "Get-CimInstance Win32_Process -Filter \\"name='${safeName}'\\" | Select-Object ProcessId,CommandLine | ConvertTo-Json"`;
        } else {
            // WMIC 命令(传统方式)
            return `wmic process where "name='${safeName}'" get ProcessId,CommandLine /format:list`;
        }
    }
