...
async function makeRequest(
  ...
  // 先尝试 HTTPS，失败后（特定错误）可按配置回退 HTTP
  try {
    return await doRequest(true, port);
  } catch (error: any) {
    const msg = (error?.message || '').toLowerCase();
    const shouldRetryHttp = allowHttpFallback && httpPort !== undefined && (error.code === 'EPROTO' || msg.includes('wrong_version_number'));
    if (shouldRetryHttp) {
      console.warn('⚠️ HTTPS connection failed. Attempting HTTP fallback (less secure)...');
      console.warn('   Error details:', error.code || error.message);
      console.warn('   HTTP fallback port:', httpPort);
...
export class QuotaService {
  ...
  private isPollingTransition: boolean = false;  // 轮询状态切换锁，防止竞争条件
  private csrfToken?: string;
  private apiMethod: QuotaApiMethod = QuotaApiMethod.GET_USER_STATUS;
  private allowHttpFallback: boolean = false;

  constructor(port: number, csrfToken?: string, httpPort?: number, allowHttpFallback: boolean = false) {
    this.port = port;
  ...
  setPort(port: number): void {
    ...
    this.retryCount = 0;
    this.httpFallbackCount = 0; // 重置 HTTP 降级计数
  }
  ...
  /**
   * 公开方法：获取当前配额数据（用于复制到剪贴板等功能）
   * 返回 QuotaSnapshot 或 null
   */
  public async fetchQuotaData(): Promise<QuotaSnapshot | null> {
    try {
      ...
      switch (this.apiMethod) {
        case QuotaApiMethod.GET_USER_STATUS: {
          ...
          if (invalid1) {
            console.error('Response code invalid; cannot fetch quota data', invalid1);
            return null;
          }
        ...
        default: {
          ...
          if (invalid2) {
            console.error('Response code invalid; cannot fetch quota data', invalid2);
            return null;
          }
  ...
  /**
   * Sanitize token for logging (show only first 6 and last 4 characters)
   */
  ...
  private async makeGetUserStatusRequest(): Promise<any> {
    console.log('Using CSRF token:', this.sanitizeToken(this.csrfToken));
    return makeRequest(
      {
